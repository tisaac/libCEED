...
// Basis Apply L2
//------------------------------------------------------------------------------
static int CeedBasisApplyL2_Ref(CeedBasis basis, CeedInt num_elem,
                                CeedTransposeMode t_mode, CeedEvalMode eval_mode,
                                CeedVector U, CeedVector V) {
  int ierr;
  Ceed ceed;
  ...
  if (tensor_basis) {
    return CeedError(ceed, CEED_ERROR_BACKEND,
                     "L2 is not supported for tensor basis");
  } else {
    // Non-tensor basis L2
    switch (eval_mode) {
    // Interpolate to/from quadrature points
...
//------------------------------------------------------------------------------
// Basis Create Non-Tensor L2
//------------------------------------------------------------------------------
int CeedBasisCreateL2_Ref(CeedElemTopology topo, CeedInt dim,
                          CeedInt num_nodes, CeedInt num_qpts,
                          const CeedScalar *interp,
                          const CeedScalar *q_ref,
                          const CeedScalar *q_weight,
  ...
  ierr = CeedSetBackendFunction(ceed, "Basis", basis, "Apply",
                                CeedBasisApplyL2_Ref); CeedChkBackend(ierr);
